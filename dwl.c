...
closemon(Monitor *m)
{
    ...
    wl_list_for_each(c, &clients, link) {
        if (c->isfloating && c->geom.x > m->m.width)
            resize(c, (struct wlr_box) {
            .x = c->geom.x - m->w.width, .y = c->geom.y,
...
monocle(Monitor *m)
{
    ...
    wl_list_for_each(c, &clients, link) {
        ...
        if (!monoclegaps)
            ...
            resize(c, (struct wlr_box) {
            .x = m->w.x + gappoh, .y = m->w.y + gappov,
            .width = m->w.width - 2 * gappoh, .height = m->w.height - 2 * gappov
        }, 0);
    }
    if (n)
             ...
             double dx_unaccel, double dy_unaccel)
{
    ...
    if (cursor_mode == CurMove) {
        ...
        resize(grabc, (struct wlr_box) {
            .x = (int)round(cursor->x) - grabcx, .y = (int)round(cursor->y) - grabcy,
            .width = grabc->geom.width, .height = grabc->geom.height
        }, 1);
        return;
    } else if (cursor_mode == CurResize) {
        resize(grabc, (struct wlr_box) {
            .x = grabc->geom.x, .y = grabc->geom.y,
            .width = (int)round(cursor->x) - grabc->geom.x, .height = (int)round(cursor->y) - grabc->geom.y
        }, 1);
        return;
    }
...
tile(Monitor *m)
{
    ...
    wl_list_for_each(c, &clients, link) {
        ...
        if (i < m->nmaster) {
            ...
            resize(c, (struct wlr_box) {
                .x = m->w.x + m->gappov*oe, .y = m->w.y + my,
            ...
            resize(c, (struct wlr_box) {
                .x = m->w.x + mw + m->gappov*oe, .y = m->w.y + ty,
                .width = m->w.width - mw - 2*m->gappov*oe, .height = h
            }, 0);
